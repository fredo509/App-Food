class InventoriesController < ApplicationController
  def index
    ...
    @inventories = Inventory.where(user_id: current_user.id).page(params[:page]).per(3)
  end

  def show
    @inventory = Inventory.find(params[:id])
    @inventory_food_inventories = @inventory.food_inventories.page(params[:page]).per(3)
  end
